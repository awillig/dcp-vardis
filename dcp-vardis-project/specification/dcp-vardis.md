...
- The transmissible data type `VarIdT` (short for variable
...
- The transmissible data type `VarRepCntT` is an 8-bit unsigned integer,
...
- The transmissible data type `VarSeqnoT` is an 8-bit unsigned integer,
...
- `VARDISPAR_MAX_VALUE_LENGTH` is the maximum allowable length of a
  ...
  value representable in the `VarLenT` data type. 
...
`VarCreateT` record together with the header of the
...
the size of the `IEHeaderT`). In this calculation, when `Q` is the
queue under consideration (which has entries of type `VarIdT`), each
...
knows how to calculate the size of a record for a given `VarIdT` (this
