...
- The transmissible data type `ExtendedSafetyDataT` acts as a wrapper
  ...
  `ExtendedSafetyDataT` and their wholesale storage in the neighbour
  table.  An `ExtendedSafetyDataT` record contains the following fields:
    ...
    - `tStamp` of type `TimeStampT` (see Section [Data
      ...
      `SafetyDataT` record is handed down by applications for
      transmission 
...
`currentSequenceNumber` of type `SRPSequenceNumberT`.
...
- `SRPPAR_NEIGHBOUR_TABLE_TIMEOUT` in milliseconds: if a node doesn't
  receive a new `ExtendedSafetyDataT` from a particular neighbour for
