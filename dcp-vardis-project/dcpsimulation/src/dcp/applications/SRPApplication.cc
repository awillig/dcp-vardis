...
{
    SRPClientProtocol::initialize(stage);

    if (stage == INITSTAGE_LAST)
    {
        dbg_setModuleName("SRPApplication");
        ...
        if (isActive)
        {
            dbg_string ("application is active");

            // create and schedule self-messages
            sampleMsg = new cMessage("SRPApplication:sampleMsg");
            ...
            ssLc << "srpapplication[" << getOwnNodeId() << "]";
            ssUc << "SRPApplication[" << getOwnNodeId() << "]";
...
void SRPApplication::handleMessage(cMessage *msg)
{
    dbg_assertToplevel();
    ...
    if ((msg->arrivedOn(gidFromSRP)) && dynamic_cast<SRPUpdateSafetyData_Confirm*>(msg))
    {
    ...
    error("SRPApplication::handleMessage: unknown message type");
...
SRPApplication::~SRPApplication ()
{
    cancelAndDelete(sampleMsg);
}


void SRPApplication::handleSampleMsg ()
{
 ...
 * position
 */
void SRPApplication::findModulePointers (void)
{
...
void SRPApplication::handleSRPUpdateSafetyDataConfirm (SRPUpdateSafetyData_Confirm* srpConf)
{
    ...
    {
        error("SRPApplication::handleSRPUpdateSafetyDataConfirm: Not SRP_STATUS_OK");
    }
