...
RES_DIR = "/local/spe107/BulkRuns_Nov22/Experiment3ResBulk"
node_cnt_x = list(range(5, 21))
separation = [250, 255, 260, 280, 263, 273]
beacon_period = [100, 50]
num_repetitions = [1, 2, 3]
num_summaries = [3, 10, 20]
update_periods = [200, 300, 400, 500, 750, 1000, 1500, 2000]

seqno_res_file = open(f"{RES_DIR}/../avg_seqno_delta.csv", "w")
seqno_res_file.write("grid_size,separation,beaconperiod,repetitions,summaries,update_period,avgdelta,stddev,cnt\n")
delay_res_file = open(f"{RES_DIR}/../avg_update_delay.csv", "w")
delay_res_file.write("grid_size,separation,beaconperiod,repetitions,summaries,update_period,avgdelay,stddev,cnt\n")

seqno_res_file_select_pos = open(f"{RES_DIR}/../avg_seqno_delta_pos.csv", "w")
seqno_res_file_select_pos.write("grid_size,separation,beaconperiod,repetitions,summaries,update_period,node_pos,avgdelta,stddev,cnt\n")
delay_res_file_select_pos = open(f"{RES_DIR}/../avg_update_delay_pos.csv", "w")
delay_res_file_select_pos.write("grid_size,separation,beaconperiod,repetitions,summaries,update_period,node_pos,avgdelay,stddev,cnt\n")

for (sep, bec_per, rep, num_sum, update_period) in itertools.product(separation, beacon_period, num_repetitions, num_summaries, update_periods):
    ...
    for n in range(len(node_cnt_x)):
        ...
        filelist = list(glob.iglob(f"{RES_DIR}/SmallGrids-LabRun-nodeCntX={num_nodes},nodeCnt={num_nodes}_#5e_2,separation={sep},beaconPeriod={bec_per}ms,numRepetitions={rep},numSummaries={num_sum},updatePeriod=exponential({update_period}),*.sca"))
        ...
        if len(seqno_delta_average_data) > 2:
            ...
            seqno_res_file.write(f"{num_nodes},{sep},{bec_per},{rep},{num_sum},{update_period},{avg},{std},{cnt}\n")
        ...
        if len(delay_average_data) > 2:
            ...
            for (avg2, std2, cnt2) in delay_average_data[1:]:
                avg, std, cnt = combine_samples(avg, std, cnt, avg2, std2, cnt2)
            delay_res_file.write(f"{num_nodes},{sep},{bec_per},{rep},{num_sum},{update_period},{avg},{std},{cnt}\n")
        ...
        if len(seqno_delta_average_data_edge) > 2:
            ...
            for (avg2, std2, cnt2) in seqno_delta_average_data_edge[1:]:
                avg, std, cnt = combine_samples(avg, std, cnt, avg2, std2, cnt2)
            seqno_res_file_select_pos.write(f"{num_nodes},{sep},{bec_per},{rep},{num_sum},{update_period},edge,{avg},{std},{cnt}\n")
        ...
        if len(delay_average_data_edge) > 2:
            ...
            for (avg2, std2, cnt2) in delay_average_data_edge[1:]:
                avg, std, cnt = combine_samples(avg, std, cnt, avg2, std2, cnt2)
            delay_res_file_select_pos.write(f"{num_nodes},{sep},{bec_per},{rep},{num_sum},{update_period},edge,{avg},{std},{cnt}\n")
        ...
        if len(seqno_delta_average_data_center) > 2:
            ...
            for (avg2, std2, cnt2) in seqno_delta_average_data_center[1:]:
                avg, std, cnt = combine_samples(avg, std, cnt, avg2, std2, cnt2)
            seqno_res_file_select_pos.write(f"{num_nodes},{sep},{bec_per},{rep},{num_sum},{update_period},center,{avg},{std},{cnt}\n")
            seqno_res_file_select_pos.flush()
        ...
        if len(delay_average_data_center) > 2:
            ...
            for (avg2, std2, cnt2) in delay_average_data_center[1:]:
                avg, std, cnt = combine_samples(avg, std, cnt, avg2, std2, cnt2)
            delay_res_file_select_pos.write(f"{num_nodes},{sep},{bec_per},{rep},{num_sum},{update_period},center,{avg},{std},{cnt}\n")
            delay_res_file_select_pos.flush()
        ...
        if len(seqno_delta_average_data_intermediate) > 2:
            ...
            for (avg2, std2, cnt2) in seqno_delta_average_data_intermediate[1:]:
                avg, std, cnt = combine_samples(avg, std, cnt, avg2, std2, cnt2)
            seqno_res_file_select_pos.write(f"{num_nodes},{sep},{bec_per},{rep},{num_sum},{update_period},intermediate,{avg},{std},{cnt}\n")
            seqno_res_file_select_pos.flush()
        ...
        if len(delay_average_data_intermediate) > 2:
            ...
            for (avg2, std2, cnt2) in delay_average_data_intermediate[1:]:
                avg, std, cnt = combine_samples(avg, std, cnt, avg2, std2, cnt2)
            delay_res_file_select_pos.write(f"{num_nodes},{sep},{bec_per},{rep},{num_sum},{update_period},intermediate,{avg},{std},{cnt}\n")
            delay_res_file_select_pos.flush()
