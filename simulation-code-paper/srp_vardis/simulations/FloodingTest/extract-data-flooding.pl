...
for my $nodecount (@nodecounts) {
	...
	for my $separation (@separations) {
		...
		$matchpat = "Flooding-numRepetitions=$numrep,updatePeriod=exponential\\($period\\),separation=$separation,nodeCntX=$nodecount,.*\\.sca";
		#print("$matchpat\n");
		my $count         = 0;
		...
		for my $file (@files) {
			...
			$seqnoStr = `opp_scavetool -l $targetdir$file | grep "nodes.0..sink.*seqnoDelta" | grep -v histogram`;
			chop($seqnoStr);
			...
			$delayStr = `opp_scavetool -l $targetdir$file | grep "nodes.0..sink.*updateDelay" | grep -v histogram`;
			chop($delayStr);
